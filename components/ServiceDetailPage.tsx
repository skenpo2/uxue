...
export default function ServiceDetailPage({ service }: { service: Service }) {
  ...
  return (
    <>
      <PageHero eyebrow={service.eyebrow} title={service.name} description={service.description} images={[service.image, ...relatedProjects.map((item) => item.image)]} breadcrumbs={[{ label: 'Solutions', href: '/solutions' }, { label: service.name }]} />
      ...
      <Section className="section">
        <div className="detail-intro"><aside className="detail-intro__aside"><p className="eyebrow">The requirement</p><h3>Infrastructure should strengthen the operation, not add another disconnected system.</h3></aside><div><h2>Designed around how your organisation needs to work.</h2><p className="lede">{service.summary} UXUE begins with the site, users, dependencies and operational outcome before defining equipment.</p></div></div>
      </Section>
      <Section className="section section--paper">
        <div className="section-heading"><div><p className="eyebrow">Capabilities</p><h2>The critical layers of {service.shortName.toLowerCase()}.</h2></div><p>Capability selection and system depth are established through assessment; not every deployment requires every layer.</p></div>
      ...
      <section className="image-split"><div className="image-split__image"><Image src={service.image} alt={`${service.name} infrastructure`} fill sizes="(max-width: 767px) 100vw, 50vw" /></div><div className="image-split__content"><div><p className="eyebrow">Technology approach</p><h2>Integrated without unnecessary complexity.</h2><p className="lede">The goal is a maintainable environment with clear dependencies, appropriate control and a handover the operating team can use.</p></div></div></section>
      <Section className="section section--dark">
        <div className="section-heading"><div><p className="eyebrow eyebrow--light">Implementation</p><h2>A disciplined path to deployment.</h2></div><p className="on-dark">Each engagement is adapted to the site and scope while following a clear technical sequence.</p></div>
      ...
      <Section className="section">
        <div className="section-heading"><div><p className="eyebrow">Where it applies</p><h2>Relevant operating environments.</h2></div><div className="related-links">{relatedIndustries.map((item) => <Link href={`/industries/${item.slug}`} key={item.slug}>{item.name}</Link>)}</div></div>
