...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const { slug } = await params; const item = getIndustry(slug); return item ? pageMetadata(`${item.name} Technology Infrastructure`, item.summary, `/industries/${slug}`, item.image) : {}; }
