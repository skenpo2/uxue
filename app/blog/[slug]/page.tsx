...
export default async function BlogPostPage({ params }: Props) {
  ...
  return <article><header className="article-hero"><div className="shell"><Breadcrumbs items={[{ label: 'Insights', href: '/blog' }, { label: post.title }]} /><h1>{post.title}</h1><p className="lede">{post.excerpt}</p><div className="article-meta"><span>By {post.author}</span><time dateTime={post.date}>{new Date(post.date).toLocaleDateString('en-KE', { dateStyle: 'long' })}</time><span>{post.readingTime}</span></div></div></header>
    ...
    <Section className="section section--paper"><div className="section-heading"><div><h2>Related perspectives.</h2></div></div><div className="article-grid">{relatedPosts.map((item) => <BlogCard key={item.slug} post={item} />)}</div></Section><CTASection /></article>;
}
